...
static char cap_oper_desc[] = "Provides the foxcomet.chat/oper capability";
...
mapi_cap_list_av2 cap_oper_caps[] = {
	{ MAPI_CAP_CLIENT, "foxcomet.chat/oper", NULL, &CLICAP_OPER },
...
cap_oper_outbound_msgbuf(void *data_)
{
	...
	if (IsOper(data->client))
	{
		...
		msgbuf_append_tag(msgbuf, "foxcomet.chat/oper", data->client->user->opername, CLICAP_OPER_AUSPEX);
		if (HasPrivilege(data->client, "oper:hidden") || ConfigFileEntry.hide_opers)
			/* these people aren't allowed to see hidden opers */
			return;
		msgbuf_append_tag(msgbuf, "foxcomet.chat/oper", data->client->user->opername, CLICAP_OPER_JUSTOPER);
		msgbuf_append_tag(msgbuf, "foxcomet.chat/oper", NULL, CLICAP_OPER_NORMAL);
	}
}
