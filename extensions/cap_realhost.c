...
static char cap_realhost_desc[] = "Provides the foxcomet.chat/realhost oper-only capability";
...
mapi_cap_list_av2 cap_realhost_caps[] = {
	{ MAPI_CAP_CLIENT, "foxcomet.chat/realhost", NULL, &CLICAP_REALHOST },
...
cap_realhost_outbound_msgbuf(void *data_)
{
	...
	{
		msgbuf_append_tag(msgbuf, "foxcomet.chat/ip", data->client->sockhost,
				IsDynSpoof(data->client) ? CLICAP_OPER_REALHOST : CLICAP_REALHOST);
	}

	if (!EmptyString(data->client->orighost))
		msgbuf_append_tag(msgbuf, "foxcomet.chat/realhost", data->client->orighost, CLICAP_OPER_REALHOST);
}
